...
class DailySales(models.Model):
    ...
    total_orders = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Всего заказов')
    )
    completed_orders = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Выполненные заказы')
    )
    cancelled_orders = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Отмененные заказы')
    )
    gross_sales = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        default=Decimal('0.00'),
        verbose_name=_('Валовые продажи')
    )
    net_sales = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        default=Decimal('0.00'),
        verbose_name=_('Чистые продажи')
    )
    discount_amount = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        default=Decimal('0.00'),
        verbose_name=_('Сумма скидок')
    )
    tax_amount = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        default=Decimal('0.00'),
        verbose_name=_('Сумма налогов')
    )
    ...
    peak_hour_orders = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Пиковый час (заказы)')
    )
    peak_hour_sales = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Пиковый час (продажи)')
    )
    ...
    def generate_for_date(cls, date, restaurant=None):
        ...
        for rest in restaurants:
            ...
            gross_sales = completed_orders_qs.aggregate(sum=Sum('subtotal'))['sum'] or Decimal('0.00')
            discount_amount = completed_orders_qs.aggregate(sum=Sum('discount_amount'))['sum'] or Decimal('0.00')
            tax_amount = completed_orders_qs.aggregate(sum=Sum('tax_amount'))['sum'] or Decimal('0.00')
            net_sales = completed_orders_qs.aggregate(sum=Sum('total_amount'))['sum'] or Decimal('0.00')
            ...
            peak_hour_orders = max(hourly_orders.items(), key=lambda x: x[1])[0] if hourly_orders else 0
            peak_hour_sales = max(hourly_sales.items(), key=lambda x: x[1])[0] if hourly_sales else 0
...
class PopularItem(models.Model):
    ...
    date_period = models.CharField(
        ...
        verbose_name=_('Период (год-месяц)')
    )
    total_orders = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Количество заказов')
    )
    total_quantity = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Общее количество порций')
    )
    total_revenue = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        default=Decimal('0.00'),
        verbose_name=_('Общая выручка')
    )
    average_rating = models.DecimalField(
        ...
        null=True,
        blank=True,
        verbose_name=_('Средняя оценка')
    )
    last_ordered = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name=_('Последний заказ')
    )
    updated_at = models.DateTimeField(
        auto_now=True,
        verbose_name=_('Дата обновления')
...
class StaffPerformance(models.Model):
    ...
    items_prepared = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Приготовлено блюд')
    )
    ...
    hours_worked = models.DecimalField(
        ...
        decimal_places=2,
        default=Decimal('0.00'),
        verbose_name=_('Отработанные часы')
    )
    updated_at = models.DateTimeField(
        auto_now=True,
        verbose_name=_('Дата обновления')
...
class TableOccupancy(models.Model):
    ...
    total_guests = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Всего гостей')
    )
    usage_minutes = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Время использования (мин)')
    )
    peak_usage_hour = models.PositiveSmallIntegerField(
        null=True,
        blank=True,
        verbose_name=_('Пиковый час использования')
    )
    updated_at = models.DateTimeField(
        auto_now=True,
        verbose_name=_('Дата обновления')
...
class NutritionalAnalytics(models.Model):
    ...
    vegetarian_orders = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Вегетарианские заказы')
    )
    vegan_orders = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Веганские заказы')
    )
    gluten_free_orders = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Безглютеновые заказы')
    )
    lactose_free_orders = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Безлактозные заказы')
    )
    ...
    def generate_for_period(cls, year_month, restaurant=None):
        ...
        for rest in restaurants:
            ...
            sorted_allergens = dict(sorted(allergens_count.items(), key=lambda x: x[1], reverse=True)[:10])
...
class CustomerSegment(models.Model):
    ...
    name = models.CharField(
        max_length=100,
        verbose_name=_('Название сегмента')
    )
    ...
    def get_total_revenue(self):
        ...
        return Order.objects.filter(
            customer__in=self.customers.all(),
            status=Order.OrderStatus.COMPLETED
...
class CustomerInsight(models.Model):
    ...
    first_visit_date = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name=_('Дата первого визита')
    )
    last_visit_date = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name=_('Дата последнего визита')
    )
    total_visits = models.PositiveIntegerField(
        default=0,
        verbose_name=_('Всего визитов')
    )
    total_spend = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        default=Decimal('0.00'),
        verbose_name=_('Всего потрачено')
    )
    avg_order_value = models.DecimalField(
        max_digits=10,
        decimal_places=2,
