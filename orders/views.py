...
from .forms import (
    OrderTypeForm, DiscountForm,
    PaymentForm, CheckoutForm, DiscountAdminForm
)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            order = Order.objects.create(
                ...
                customer_phone=request.user.profile.phone_number if hasattr(request.user, 'profile') else None,
        ...
        if current_order_id:
            try:
                current_order = Order.objects.get(id=current_order_id, status=Order.OrderStatus.DRAFT)
...
def customer_menu_selection(request):
    ...
    try:
        order = Order.objects.get(id=current_order_id, status=Order.OrderStatus.DRAFT, customer=request.user)
    except Order.DoesNotExist:
        messages.error(request, _('Заказ не найден. Пожалуйста, начните заново.'))
        request.session.pop('current_order_id', None)
        return redirect('customer_order_create')
...
def customer_checkout(request):
    ...
    if request.method == 'POST':
        form = CheckoutForm(request.POST, instance=order, user=request.user, order_type=order.order_type)
        if form.is_valid():
            ...
            messages.success(request, _('Ваш заказ успешно оформлен! Номер заказа: {}.').format(order.order_number))
        ...
        form = CheckoutForm(instance=order, user=request.user, order_type=order.order_type, initial=initial_data)
...
def active_orders(request):
    ...
    orders = Order.objects.filter(
        ...
        status__in=[Order.OrderStatus.DRAFT, Order.OrderStatus.COMPLETED, Order.OrderStatus.CANCELLED]
    ).order_by('-placed_at')
    ...
    waiters = User.objects.filter(
        Q(role='WAITER') &
        (Q(restaurant=restaurant) | Q(managed_restaurants=restaurant))
    ).distinct()
...
def create_order(request, table_id=None):
    ...
    if user.restaurant:
        ...
        if 'restaurant_id' in request.GET:
            restaurant = get_object_or_404(Restaurant, id=request.GET['restaurant_id'], is_active=True)
...
def edit_order(request, id):
    ...
    if request.method == 'POST' and 'add_item' in request.POST:
        ...
        try:
            menu_item = MenuItem.objects.get(id=menu_item_id, restaurant=order.restaurant, is_active=True)
...
def order_history(request):
    ...
    order_stats = {
        ...
        'avg_order_value': completed_orders.aggregate(Avg('total_amount'))['total_amount__avg'] or 0,
    }
...
def discount_list(request):
    ...
    if user.is_admin():
        ...
        restaurants = user.managed_restaurants.filter(is_active=True)
        discounts = Discount.objects.filter(Q(restaurant__in=restaurants) | Q(restaurant__isnull=True))
    else:
        if user.restaurant:
            restaurants = Restaurant.objects.filter(id=user.restaurant.id)
            discounts = Discount.objects.filter(Q(restaurant=user.restaurant) | Q(restaurant__isnull=True))
        else:
            messages.error(request, _('Вы не привязаны ни к одному ресторану.'))
            return redirect('home')
...
def discount_edit(request, id):
    ...
    usage_stats = {
        ...
        'total_discount_amount': discount.orders.aggregate(Sum('discount_amount'))['discount_amount__sum'] or 0,
    }
...
def discount_toggle_status(request, id):
    ...
    messages.success(request, _('Статус скидки/промокода изменен на: {}').format(
...
def discount_delete(request, id):
    ...
    active_orders_with_discount = Order.objects.filter(
        discount=discount,
        status__in=[Order.OrderStatus.DRAFT, Order.OrderStatus.PLACED, Order.OrderStatus.PREPARING,
                    Order.OrderStatus.READY, Order.OrderStatus.SERVED]
    )

    if active_orders_with_discount.exists():
        messages.error(request, _('Невозможно удалить скидку/промокод, так как она используется в активных заказах.'))
