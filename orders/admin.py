...
class DiscountAdmin(admin.ModelAdmin):
    list_display = (
    'code', 'name', 'discount_type', 'value', 'is_valid_display', 'valid_from', 'valid_to', 'times_used', 'is_active')
    ...
    fieldsets = (
        (None, {
            'fields': ('code', 'name', 'description', 'discount_type', 'value', 'is_active')
        }),
        (_('Ограничения'), {
            'fields': ('min_order_value', 'max_discount_amount', 'valid_from', 'valid_to', 'usage_limit', 'times_used')
        }),
        (_('Применение'), {
...
class OrderAdmin(admin.ModelAdmin):
    list_display = ('order_number', 'restaurant', 'get_table_display', 'waiter', 'status', 'order_type', 'total_amount',
    ...
    readonly_fields = ('order_number', 'created_at', 'updated_at', 'placed_at', 'completed_at',
                       'subtotal', 'tax_amount', 'discount_amount', 'total_amount',
                       'display_nutrition', 'display_allergens')
    inlines = [OrderItemInline, PaymentInline]
    actions = ['mark_as_placed', 'mark_as_preparing', 'mark_as_ready', 'mark_as_served', 'mark_as_completed',
               'mark_as_cancelled']
    fieldsets = (
        (None, {
            'fields': ('order_number', 'restaurant', 'table', 'waiter', 'customer', 'status', 'order_type')
        }),
        (_('Информация о клиенте'), {
            'fields': ('customer_name', 'customer_phone', 'delivery_address', 'special_requests'),
            'classes': ('collapse',) if not Order._meta.get_field('delivery_address') else tuple(),
        }),
        (_('Суммы'), {
    ...
    def payment_status(self, obj):
        total_paid = sum(p.amount for p in obj.payments.filter(status=Payment.PaymentStatus.COMPLETED))
        ...
        if total_paid == 0:
            ...
            return format_html('<span style="color: orange;">Частично оплачен ({:.0f}%)</span>', percentage)
        else:
    ...
    def mark_as_cancelled(self, request, queryset):
        queryset.exclude(status=Order.OrderStatus.COMPLETED).update(status=Order.OrderStatus.CANCELLED)
...
class OrderItemAdmin(admin.ModelAdmin):
    list_display = ('__str__', 'order', 'menu_item', 'quantity', 'unit_price', 'get_total_price', 'status')
    ...
    fieldsets = (
        (None, {
            'fields': ('order', 'menu_item', 'quantity', 'unit_price', 'status')
        }),
...
class PaymentAdmin(admin.ModelAdmin):
    list_display = ('id', 'order', 'payment_method', 'amount', 'status', 'payment_date', 'refund_amount')
    list_filter = ('status', 'payment_method', 'payment_date')
    search_fields = ('order__order_number', 'transaction_id', 'notes')
    readonly_fields = ('created_at', 'updated_at')
    fieldsets = (
        (None, {
            'fields': ('order', 'payment_method', 'amount', 'status', 'payment_date')
        }),
    ...
    def process_refund(self, request, queryset):
        ...
        if queryset.count() != 1:
            self.message_user(request, _('Выберите только один платеж для возврата.'), level=messages.ERROR)
            return

        payment = queryset.first()
        if payment.status not in [Payment.PaymentStatus.COMPLETED, Payment.PaymentStatus.PARTIAL_REFUND]:
            self.message_user(request, _('Возврат возможен только для выполненных платежей.'), level=messages.ERROR)
            return

        return redirect(f'/admin/orders/payment/{payment.id}/refund/')  # TODO когда реализую урлы, приду сюда и сделаю норм ссылку с ленивой загрузкой
