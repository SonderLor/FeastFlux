...
class OrderTypeForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user and hasattr(user, 'profile') and user.profile.saved_delivery_addresses:
            ...
            self.fields['saved_address'] = forms.ChoiceField(
                choices=[('', _('Выберите сохраненный адрес'))] + [(addr, addr) for addr in saved_addresses],
                required=False,
                widget=forms.Select(attrs={'class': 'form-select'}),
    ...
    def clean(self):
        ...
        if order_type == Order.OrderType.DINE_IN:
            if not restaurant:
                self.add_error('restaurant', _('Для заказа в ресторане необходимо выбрать ресторан'))
            if not table:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.menu_item:
            modifiers = Modifier.objects.filter(
                restaurant=self.menu_item.restaurant,
                is_active=True
            ).filter(
                Q(applicable_items=self.menu_item) |
                Q(applicable_items__isnull=True)
            ).distinct()
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            for field_name, value in self.cleaned_data.items():
                if field_name.startswith('modifier_') and not field_name.endswith('_quantity') and value:
...
class DiscountAdminForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'min_order_value', 'max_discount_amount', 'valid_from', 'valid_to',
            'usage_limit', 'restaurant', 'applicable_categories', 'applicable_items',
            'free_item', 'is_active'
        ]
        widgets = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk and self.instance.restaurant:
            self.fields['applicable_categories'].queryset = self.instance.restaurant.menu_categories.filter(
                is_active=True)
            self.fields['applicable_items'].queryset = self.instance.restaurant.menu_items.filter(is_active=True)
            self.fields['free_item'].queryset = self.instance.restaurant.menu_items.filter(is_active=True)
