...
class Order(models.Model):
    ...
    def __str__(self):
        if self.table:
            ...
            return (
                f"Заказ №{self.id} ({self.get_order_type_display()}) - {self.get_status_display()}"
            )
...
class OrderItem(models.Model):
    ...
    item_name_at_order = models.CharField(
        max_length=200, blank=True, verbose_name=_("Название на момент заказа")
    )
    nutrition_at_order = models.JSONField(
        null=True, blank=True, verbose_name=_("КБЖУ на момент заказа")
    )
    modifiers = models.ManyToManyField(Modifier, blank=True, verbose_name=_("Модификаторы"))

    comment = models.CharField(
        max_length=255, blank=True, null=True, verbose_name=_("Комментарий к позиции")
    )
