...
class Discount(models.Model):
    ...
    code = models.CharField(
        max_length=50,
        unique=True,
        verbose_name=_('Код скидки/промокод')
    )
...
class Order(models.Model):
    ...
    placed_at = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name=_('Время размещения')
    )
...
class OrderItem(models.Model):
    ...
    def get_total_price(self):
        ...
        modifier_cost = sum(mod.modifier.price_change * mod.quantity for mod in self.order_item_modifiers.all())
...
class Payment(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.status == self.PaymentStatus.COMPLETED:
            total_paid = sum(p.amount for p in self.order.payments.filter(status=self.PaymentStatus.COMPLETED))

            if total_paid >= self.order.total_amount and self.order.status in [Order.OrderStatus.SERVED,
