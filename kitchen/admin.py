...
class KitchenOrderItemInline(admin.TabularInline):
    ...
    extra = 0
    fields = ('order_item', 'status', 'cooking_station', 'assigned_to',
              'sequence_number', 'estimated_cooking_time', 'get_cooking_time', 'is_delayed')
...
class KitchenQueueAdmin(admin.ModelAdmin):
    list_display = ('name', 'restaurant', 'status', 'is_default', 'active_orders_count', 'average_processing_time')
    list_filter = ('restaurant', 'status', 'is_default')
    search_fields = ('name', 'description', 'restaurant__name')
    readonly_fields = ('created_at', 'updated_at')
    fieldsets = (
        (None, {
            'fields': ('restaurant', 'name', 'description', 'status', 'is_default', 'order')
        }),
        (_('Ответственность'), {
            'fields': ('responsible_roles',)
        }),
        (_('Система'), {
...
class KitchenOrderAdmin(admin.ModelAdmin):
    list_display = ('order', 'queue', 'status', 'priority', 'assigned_to',
                    'created_at', 'get_processing_time', 'is_delayed_display')
    list_filter = ('queue', 'status', 'priority', 'assigned_to')
    search_fields = ('order__order_number', 'notes', 'assigned_to__username')
    readonly_fields = ('created_at', 'started_at', 'completed_at', 'get_processing_time', 'is_delayed_display')
    inlines = [KitchenOrderItemInline, KitchenEventInline]
    fieldsets = (
        (None, {
            'fields': ('queue', 'order', 'status', 'priority', 'assigned_to')
        }),
        (_('Дополнительная информация'), {
            'fields': ('notes', 'estimated_completion_time')
        }),
        (_('Временные отметки'), {
            'fields': ('created_at', 'started_at', 'completed_at', 'get_processing_time', 'is_delayed_display'),
            'classes': ('collapse',)
        }),
    )
    ...
    def mark_as_delayed(self, request, queryset):
        for order in queryset.exclude(
                status__in=[KitchenOrder.KitchenOrderStatus.COMPLETED, KitchenOrder.KitchenOrderStatus.CANCELLED]):
...
class KitchenOrderItemAdmin(admin.ModelAdmin):
    list_display = ('order_item', 'kitchen_order', 'status', 'cooking_station',
                    'assigned_to', 'estimated_cooking_time', 'get_cooking_time', 'is_delayed_display')
    list_filter = ('status', 'cooking_station', 'assigned_to')
    search_fields = ('order_item__menu_item__name', 'preparation_notes')
    readonly_fields = ('started_at', 'completed_at', 'get_cooking_time', 'is_delayed_display',
                       'kitchen_order', 'order_item')
    fieldsets = (
        (None, {
            'fields': ('kitchen_order', 'order_item', 'status', 'sequence_number')
        }),
        (_('Приготовление'), {
            'fields': ('cooking_station', 'assigned_to', 'estimated_cooking_time', 'preparation_notes')
        }),
        (_('Временные отметки'), {
...
class CookingStationAdmin(admin.ModelAdmin):
    list_display = ('name', 'restaurant', 'queue', 'is_active', 'get_current_workload', 'average_prep_time')
    list_filter = ('restaurant', 'is_active', 'queue')
    search_fields = ('name', 'description', 'restaurant__name')
    readonly_fields = ('created_at', 'updated_at')
    fieldsets = (
        (None, {
            'fields': ('restaurant', 'name', 'description', 'is_active')
        }),
        (_('Настройки'), {
            'fields': ('queue', 'responsible_roles', 'average_prep_time')
        }),
        (_('Система'), {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class KitchenEventAdmin(admin.ModelAdmin):
    list_display = ('event_type', 'restaurant', 'get_order_number', 'get_item_name', 'created_by', 'created_at')
    list_filter = ('restaurant', 'event_type', 'created_at')
    search_fields = ('description', 'kitchen_order__order__order_number')
    readonly_fields = ('restaurant', 'kitchen_order', 'kitchen_item', 'event_type',
                       'description', 'created_by', 'created_at', 'ip_address', 'user_agent')
    fieldsets = (
        (None, {
            'fields': ('event_type', 'description', 'created_by', 'created_at')
        }),
        (_('Связи'), {
            'fields': ('restaurant', 'kitchen_order', 'kitchen_item')
        }),
