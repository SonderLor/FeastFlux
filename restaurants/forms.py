...
class ReservationForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (reservation_date and reservation_time and
                reservation_date == timezone.now().date() and
        ...
        if self.restaurant and reservation_date and reservation_time and number_of_guests:
            ...
            if not available_tables:
                self.add_error(None,
                               _('К сожалению, нет доступных столиков на выбранное время и дату. Пожалуйста, выберите другое время или дату.'))
            else:
    ...
    def save(self, commit=True):
        ...
        if self.cleaned_data.get('reservation_time') and self.cleaned_data.get('duration'):
            ...
            end_datetime = reservation_datetime + timezone.timedelta(hours=self.cleaned_data['duration'])
...
class TableFilterForm(forms.Form):
    ...
    min_capacity = forms.IntegerField(
        ...
        widget=forms.NumberInput(attrs={'class': 'form-control', 'placeholder': _('Минимум гостей')})
    )
    max_capacity = forms.IntegerField(
        required=False,
        min_value=1,
        label=_('Максимальная вместимость'),
        widget=forms.NumberInput(attrs={'class': 'form-control', 'placeholder': _('Максимум гостей')})
    )
...
class RestaurantFilterForm(forms.Form):
    ...
    search = forms.CharField(
        required=False,
        max_length=100,
        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': _('Поиск по названию или адресу')}),
        label=_('Поиск')
    )
