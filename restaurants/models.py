...
class Restaurant(models.Model):
    ...
    name = models.CharField(max_length=100, verbose_name=_("Название ресторана"))
    ...
    postal_code = models.CharField(
        max_length=20, blank=True, null=True, verbose_name=_("Почтовый индекс")
    )
    ...
    logo = models.ImageField(
        upload_to="restaurant_logos/", blank=True, null=True, verbose_name=_("Логотип")
    )
    ...
    def get_reservations_today(self):
        ...
        return self.reservations.filter(reservation_date=today)
...
class Table(models.Model):
    ...
    restaurant = models.ForeignKey(
        Restaurant, on_delete=models.CASCADE, related_name="tables", verbose_name=_("Ресторан")
    )
    number = models.PositiveIntegerField(verbose_name=_("Номер столика"))
    capacity = models.PositiveSmallIntegerField(
        default=4, validators=[MinValueValidator(1)], verbose_name=_("Вместимость")
    )
    ...
    width = models.PositiveSmallIntegerField(null=True, blank=True, verbose_name=_("Ширина (см)"))
    length = models.PositiveSmallIntegerField(null=True, blank=True, verbose_name=_("Длина (см)"))
    min_spend = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name=_("Минимальный чек")
    )
    location_description = models.CharField(
        max_length=255, blank=True, null=True, verbose_name=_("Описание расположения")
    )
    is_active = models.BooleanField(default=True, verbose_name=_("Активен"))
    position_x = models.FloatField(default=0, verbose_name=_("Позиция X на карте"))
    position_y = models.FloatField(default=0, verbose_name=_("Позиция Y на карте"))
    created_at = models.DateTimeField(auto_now_add=True, verbose_name=_("Дата создания"))
    updated_at = models.DateTimeField(auto_now=True, verbose_name=_("Дата обновления"))
    ...
    def is_available(self, reservation_date, reservation_time, duration=2):
        ...
        reservation_datetime = timezone.datetime.combine(reservation_date, reservation_time)
...
class Reservation(models.Model):
    ...
    table = models.ForeignKey(
        Table, on_delete=models.CASCADE, related_name="reservations", verbose_name=_("Столик")
    )
    ...
    number_of_guests = models.PositiveSmallIntegerField(
        default=2, validators=[MinValueValidator(1)], verbose_name=_("Количество гостей")
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.reservation_time and not self.end_time:
            reservation_datetime = timezone.datetime.combine(
                timezone.datetime.today(), self.reservation_time
            )
