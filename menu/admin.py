...
class CategoryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'description', 'parent', 'restaurant', 'is_active', 'order')
        }),
        (_('Изображение'), {
            'fields': ('image',),
            'classes': ('collapse',)
...
class IngredientAdmin(admin.ModelAdmin):
    list_display = (
    'name', 'restaurant', 'display_allergens', 'calories_per_100g', 'is_vegetarian', 'is_vegan', 'is_gluten_free')
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'description', 'restaurant', 'created_by')
        }),
        (_('Пищевая ценность (на 100г)'), {
            'fields': ('calories_per_100g', 'protein_per_100g', 'fat_per_100g', 'carbs_per_100g', 'fiber_per_100g',
                       'sugar_per_100g')
        }),
...
class MenuItemAdmin(admin.ModelAdmin):
    list_display = (
    'name', 'category', 'restaurant', 'price', 'status', 'display_nutrition', 'is_vegetarian', 'is_vegan', 'is_active')
    list_filter = (
    'restaurant', 'category', 'status', 'is_active', 'is_vegetarian', 'is_vegan', 'is_gluten_free', 'is_signature')
    search_fields = ('name', 'description')
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'description', 'category', 'restaurant', 'price', 'status', 'is_active', 'created_by')
        }),
        (_('Диетические ограничения'), {
            'fields': ('is_vegetarian', 'is_vegan', 'is_gluten_free', 'is_lactose_free', 'spicy_level')
        }),
        (_('Изображения'), {
...
class MenuItemIngredientAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('menu_item', 'ingredient', 'amount_grams')
        }),
        (_('Отображение'), {
            'fields': ('is_main', 'is_visible', 'order', 'notes')
        }),
    )
...
class ModifierAdmin(admin.ModelAdmin):
    list_display = ('name', 'restaurant', 'modifier_type', 'price_change', 'related_ingredient', 'is_active')
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'description', 'restaurant', 'modifier_type', 'is_active')
        }),
        (_('Цена и связи'), {
            'fields': ('price_change', 'related_ingredient', 'applicable_items')
        }),
        (_('Влияние на пищевую ценность'), {
            'fields': ('nutrition_impact',),
            'classes': ('collapse',)
        }),
        (_('Дополнительная информация'), {
            'fields': ('created_at', 'updated_at'),
